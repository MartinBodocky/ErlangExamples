...
open_storage() ->
	FactoryFile = get_node_name(factory),
	HistoryFile = get_node_name(history),
	%% Data storage for keep list of current sessions
	ets:new(factoryRam, [named_table, {keypos, #sRef.referenceId}]),
	dets:open_file(factoryDisk, [{file, FactoryFile}, {keypos, #sRef.referenceId}]),
	...
	dets:open_file(historyDisk, [{file, HistoryFile}, {type, duplicate_bag}, {keypos, #uHis.referenceId}]),
	%% restore storage
	restore_storage().
...
get_node_name(factory) ->
	case erlang:is_alive() of
		true -> atom_to_list(node()) ++ "_Factory";
		false -> "FactoryFile"
	end;
get_node_name(history) ->
	case erlang:is_alive() of
		true -> atom_to_list(node()) ++ "_History";
		false -> "HistoryFile"
	end.
