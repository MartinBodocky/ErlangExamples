...
start_link(UserName) ->
	...
	warmup(),
	call(factory, {add_user, UserName}).

-spec skateboard(string(),integer()) -> any().
skateboard(RefId, Count) ->
	SessionPid = call(factory, {get_session_pid, RefId}),
...
user_history(RefId) ->
	call(factory, {user_history, RefId}).
...
buy(RefId) ->
	...
	if CartCount == 0 -> {error, empty_cart};
		true ->
			case call(factory, {buy, RefId, Session}) of
				{ok, TrxId} ->
					%% save to history 
					call(factory, {add_transaction, RefId, TrxId, erlang:localtime(), Session#session.cart, Session#session.price}),
					%% delete session
					call(SessionPid, delete),
					%% delete in factory storage
					call(factory, {remove_session, RefId}),
					{ok, TrxId};
...
get_session_pid(RefId) ->
	call(factory, {get_session_pid, RefId}).

get_sessionByPid(Pid) ->
	call(factory, {get_sessionByPid, Pid}).

get_allSessions() ->
	call(factory, all_sessions).
